...
function synapse_sim(gluPerVesicle,amparNo, ratio,fileNo,period, random_number,PSD_factor,duration,release_zone,PSD, SpillOver)
...
%---- cleft geometry control constants ------------------------------------
    ...
    S.R_Reservoir           = S.R_PSD;
    
    S.R_ReleaseZone         = 0 + (release_zone - 0) * random_number; % radius of release zone, 0=center 
...
%--------------------------------------------------------------------------  
    ...
    for i = 1:2
        if i == 1 % GluA2, stands for slow-GluAs
            % AMPAR number on the PSD at time 0
            S.amparNo_PSD           = round(amparNo * S.ratio)*1 ;
            if S.amparNo_PSD == 0
                continue
            end
            S.amparNo_outside       = 0;%round(S.amparNo_PSD/(pi*S.R_PSD^2)* (pi*S.R_Reservoir^2-pi*S.R_PSD^2)* a);
            ...
            % AMPAR number on the PSD at time 0
            S.amparNo_PSD           = round(amparNo * (1 - S.ratio))*1 ;  
            if S.amparNo_PSD == 0
                continue
            end
            S.amparNo_outside       = 0; %round(S.amparNo_PSD/(pi*S.R_PSD^2)* (pi*S.R_Reservoir^2-pi*S.R_PSD^2)* a);
