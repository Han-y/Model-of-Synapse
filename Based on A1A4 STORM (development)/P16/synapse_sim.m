...
function synapse_sim(gluPerVesicle, amparNo,ratio,fileNo,period,transpDensity,D_glu,factor,...
...
%---- cleft geometry control constants ------------------------------------
    S.R_Cleft               = (Synaptic_radius_A1 + Synaptic_radius_A4)/2; %1128; % radius, nm
    ...
    S.cleftHeight           = 28; % cleft height at the calyx of Held
    S.esHeight              = 10000;
...
%---- glu control constants -----------------------------------------------
    S.D_glu                 = D_glu; %400000;   % glu diffusion constant, Franks, Sejnowski 0.2 (mu m)^2/ms = 200000 nm^2/ms, OTHER: 0.6  (mu m)^2/ms, 0.76 (mu m)^2/ms
...
%--------------------------------------------------------------------------  
    ...
    for i = 1:2
        if i == 1 % GluA1, stands for slow-GluAs
            ...
            S.amparNo = amparNo;
            amparNo_a1 = round(amparNo * S.ratio);
            S.amparNo_PSD           = round( amparNo_a1/ ratio_Num_SynNano_STORM_A1);
            S.amparNo_outside       = amparNo_a1 - S.amparNo_PSD;%round(S.amparNo_PSD/(pi*S.R_PSD^2)* (pi*S.R_Reservoir^2-pi*S.R_PSD^2)* a);
            S.R_PSD                 = Nano_radius_A1; 
            S.R_Reservoir           = Synaptic_radius_A1;
            ...
            % AMPAR number on the PSD at time 0
            S.amparNo = amparNo;
            amparNo_a4 = round(amparNo * (1 - S.ratio));
            S.amparNo_PSD           = round(amparNo_a4 / ratio_Num_SynNano_STORM_A4); 
            S.amparNo_outside       = amparNo_a4 - S.amparNo_PSD; %round(S.amparNo_PSD/(pi*S.R_PSD^2)* (pi*S.R_Reservoir^2-pi*S.R_PSD^2)* a);
            S.R_PSD                 = Nano_radius_A4; 
            S.R_Reservoir           = Synaptic_radius_A4;  
