...
%---- kinetic schemes -----------------------------------------------------
    ...
    kinetics.amparState_F(kinetics_GluA2, amparState_C0 ) = 1;%#ok  F
    kinetics.amparState_B(kinetics_GluA2, amparState_C1) = 1; %#ok  B
    kinetics.amparState_B(kinetics_GluA2, amparState_C2) = 1; %#ok  B
    kinetics.amparState_D(kinetics_GluA2, amparState_D3) = 1; %#ok  D
    kinetics.amparState_D(kinetics_GluA2, amparState_D4) = 1; %#ok  D
    kinetics.amparState_D(kinetics_GluA2, amparState_D5) = 1; %#ok  D
    kinetics.amparState_D(kinetics_GluA2, amparState_D6) = 1; %#ok  D
    kinetics.amparState_D(kinetics_GluA2, amparState_D7) = 1; %#ok  D
...
%%%% 2015-Calyx of Held
    ...
    kinetics.Q_transition(kinetics_GluA2, :, :) = Q;               %#ok
...
%                 ',vesiclesPerPulse=',   num2str(SIM.vesiclesPerPulse), ...
%                 ...%',vesicleJitter=',      num2str(SIM.vesicleJitter), ...
%                 ',d_glia=',             num2str(SIM.R_ES - SIM.R_Cleft), ...
%                 ',R_ReleaseZone=',      num2str(SIM.R_ReleaseZone), ...

%                 ',h_cleft=',            num2str(SIM.cleftHeight), ...
%                 ',Kp=',                 num2str(SIM.kineticsFrac_PSD), ...
%                 ',Ko=',                 num2str(SIM.kineticsFrac_Outside), ...
%                 ',T=',                  num2str(SIM.finalTime, '%.4f'), ...
%                 ',runs=',               num2str(SIM.runs));
...
for i_runs = runsBegin : SIM.runs
    ...
    for i_ampar = 1 : SIM.amparNo_outside
        ...
        ang = unifrnd(0, 1);
        ampar.traj(:, i_ampar +SIM.amparNo_PSD) = [rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
    ...
    for i_ampar = 1 : SIM.amparNo
        ...
        u = unifrnd(0, 1);
        ll = 0;
        ampar.kinetics(i_ampar) = kinetics_GluA2; % default (numerical reasons, DONT CHANGE this line)
