function synapse_fun_0_5_3_subregions(SIM)
...
if strcmp(SIM.group,'Ctrl')
    if strcmp(SIM.subunit,'GluA1')
        %%%%%%% A1 ctrl
            for i_ampar = 1 : 161
                rad = sqrt(unifrnd(0, 60^2)); % (Take square root of unif. distributed random value)
                ang = unifrnd(0, 1);
                ampar.traj(:, i_ampar) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
                ampar.tag(i_ampar)     = amparState_InCleft_OnPSD;
            end
            for i_ampar = 1 : 139
                rad = sqrt(unifrnd(60^2, 100^2)); % (Take square root of unif. distributed random value)
                ang = unifrnd(0, 1);
                ampar.traj(:, i_ampar + 161) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
                ampar.tag(i_ampar + 161)     = amparState_InCleft_OnPSD;
            end
            for i_ampar = 1 : 150
                rad = sqrt(unifrnd(100^2, 140^2)); % (Take square root of unif. distributed random value)
                ang = unifrnd(0, 1);
                ampar.traj(:, i_ampar + 300) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
                ampar.tag(i_ampar + 300)     = amparState_InCleft_OnPSD;
            end
    elseif strcmp(SIM.subunit,'GluA4')
        ...
        %%%%%% A4 ctrl
            ...
            for i_ampar = 1 : 170
                rad = sqrt(unifrnd(60^2, 100^2)); % (Take square root of unif. distributed random value)
                ang = unifrnd(0, 1);
                ampar.traj(:, i_ampar + 196) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
                ampar.tag(i_ampar + 196)     = amparState_InCleft_OnPSD;
            end
            for i_ampar = 1 : 184
                rad = sqrt(unifrnd(100^2, 140^2)); % (Take square root of unif. distributed random value)
                ang = unifrnd(0, 1);
                ampar.traj(:, i_ampar + 366) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
                ampar.tag(i_ampar + 366)     = amparState_InCleft_OnPSD;
            end
    end
...
elseif strcmp(SIM.group,'KO')
    if strcmp(SIM.subunit,'GluA1')
        ...
        for i_ampar = 1 : 149
            rad = sqrt(unifrnd(60^2, 100^2)); % (Take square root of unif. distributed random value)
            ang = unifrnd(0, 1);
            ampar.traj(:, i_ampar + 100) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
            ampar.tag(i_ampar + 100)     = amparState_InCleft_OnPSD;
        end
        for i_ampar = 1 : 174
            rad = sqrt(unifrnd(100^2, 140^2)); % (Take square root of unif. distributed random value)
            ang = unifrnd(0, 1);
            ampar.traj(:, i_ampar + 249) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
            ampar.tag(i_ampar + 249)     = amparState_InCleft_OnPSD;
        end
    elseif strcmp(SIM.subunit,'GluA4') 
        %%%%%%%% A4 KO
        for i_ampar = 1 : 122
        ...
        for i_ampar = 1 : 181
            rad = sqrt(unifrnd(60^2, 100^2)); % (Take square root of unif. distributed random value)
            ang = unifrnd(0, 1);
            ampar.traj(:, i_ampar + 122) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
            ampar.tag(i_ampar + 122)     = amparState_InCleft_OnPSD;
        end
        for i_ampar = 1 : 212
            rad = sqrt(unifrnd(100^2, 140^2)); % (Take square root of unif. distributed random value)
            ang = unifrnd(0, 1);
            ampar.traj(:, i_ampar + 303) = [NND + rad*cos(2*pi*ang), rad*sin(2*pi*ang), SIM.lowerLid]';
            ampar.tag(i_ampar + 303)     = amparState_InCleft_OnPSD;
        end
    end
end
